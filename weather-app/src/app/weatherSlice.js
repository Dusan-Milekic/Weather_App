...
// Thunk — prima grad kao argument
export const fetchWeather = createAsyncThunk(
  "weather/fetchWeather",
  async (locationArg, { getState, rejectWithValue }) => {
    try {
      const fallback = getState()?.weather?.city || "Belgrade";
      const location = (locationArg ?? "").trim() || fallback;
      ...
      return {
        ...
        // “flat” polja (zgodna za komponente)
        city,
        country,
...
const weatherSlice = createSlice({
  ...
  reducers: {
    ...
    setCity(state, action) {
      // opcionalno: lokalna izmena grada bez fetch-a
      state.city = (action.payload ?? "").trim() || state.city;
    },
  },
...
export const { clearWeather, setCity } = weatherSlice.actions;
